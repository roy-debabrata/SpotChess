...
public class RookAndBishopMovesUtil {
    ...
    private static void setupMoveTables() {
        setupMoveTable(PieceType.ROOK, false, rookMask, rookMagic, rookShift, maxRookShift, rookAttacksCache);
        setupMoveTable(PieceType.ROOK, true, rookMask, rookPinMagic, rookShift, maxRookShift, rookPinsCache);
        setupMoveTable(PieceType.BISHOP, false, bishopMask, bishopMagic, bishopShift, maxBishopShift, bishopAttacksCache);
        setupMoveTable(PieceType.BISHOP, true, bishopMask, bishopPinMagic, bishopShift, maxBishopShift, bishopPinsCache);
    }

    private static void setupMoveTable(PieceType piece, boolean pinningType, long [] masks, long [] magics, int[] shift, int maxShift, long [] targetCache ){
        for ( int i = 0; i < 64; i ++ ) {
            ...
            for ( int j = 0; j < pieceCombinations.length; j ++ ) {
                ...
                if ( targetCache[index] != 0 && targetCache[index] != moveCombinations[j]) {
                ...
                targetCache[index] = moveCombinations[j];
            }
        }
    }
